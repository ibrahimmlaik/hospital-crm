...
import { createLab, updateLab, deleteLab } from "@/actions/admin-labs";
...
export default function LabFormClient({ lab }: LabFormProps) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const result = isEdit
                ? await updateLab(lab!.id, formData)
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                <label className="block text-sm font-medium text-indigo-200 mb-2">Lab Name *</label>
                <input
                    name="name"
                    defaultValue={lab?.name || ""}
                    required
                    className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-indigo-300/50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all"
            ...
            <div>
                ...
                <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-indigo-300/50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all resize-none"
            ...
            {isEdit && (
                <div>
                    ...
                    <select
                        name="isActive"
                        defaultValue={lab?.isActive ? "true" : "false"}
                        className="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all"
                    >
            ...
            <div className="flex gap-4 pt-2">
                <button
                    type="submit"
                    disabled={loading}
                    className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white font-bold py-3 px-8 rounded-lg shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {loading ? "Saving..." : isEdit ? "Update Lab" : "Create Lab"}
                </button>
